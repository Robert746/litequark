...
const string strMessageMagic = "LiteQuark Signed Message:\n";
...
static const int64 nBlockRewardStartCoin = 2.50 * COIN; // 7200 LTQ per day
static const int64 nBlockRewardMinimumCoin = 0.1 * COIN; // 288 LTQ per day minimum
...
*/
        ...
        // Genesis block
        const char* pszTimestamp = "Make me proud";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LiteQuarkMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LiteQuarkMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("LiteQuarkMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("litequark-miner");
    ...
    try { while (true) {
        ...
        printf("Running LiteQuarkMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//            unsigned int nNonceFound;
    ...
    {
        printf("LiteQuarkMiner terminated\n");
        throw;
    }
}
