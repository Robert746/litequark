...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("LTQ")
    updateDisplayUnit();
}
...
{
    QString ret = QString("litequark:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non LTQ unit input in lnReqAmount, we generate the URI with LTQ as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::LTQ, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
